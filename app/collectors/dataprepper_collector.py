...
class DataPrepperScrapeResult:
    def __init__(self):
        ...
        self.ingest_health_404 = False   # True when /health returns 404 (service present but path not exposed)
        ...
        # Latency from _max gauge (rolling window – semantically correct for current latency)
        self.os_pipeline_latency_max: float = -1.0    # -1 = not present in metrics
        # Cumulative sum and count for delta-based average fallback
        self.os_pipeline_latency_sum: float = 0.0
        self.os_pipeline_latency_count: float = 0.0
        # Best-effort current latency (set after aggregation)
        self.os_pipeline_latency: float = 0.0
...
_PIPELINE_SUFFIXES = [
    "recordsProcessed_total",
    "http_requestsReceived_total",
    "http_successRequests_total",
    "http_requestTimeouts_total",
    "http_requestsRejected_total",
    "http_badRequests_total",
    "http_internalServerError_total",
    "opensearch_recordsIn_total",
    "opensearch_documentErrors_total",
    "opensearch_bulkRequestFailed_total",
    "opensearch_documentsSuccess_total",
    # Latency: prefer _max (rolling window reset), store _sum/_count for delta fallback.
    # Do NOT use the bare _seconds name – that resolves to the _sum sample (cumulative).
    "opensearch_PipelineLatency_seconds_max",
    "opensearch_PipelineLatency_seconds_sum",
    "opensearch_PipelineLatency_seconds_count",
    "BlockingBuffer_recordsInBuffer",
    "BlockingBuffer_recordsInFlight",
    "BlockingBuffer_bufferUsage",
    "BlockingBuffer_recordsWriteFailed_total",
...
def _aggregate_metrics(result):
    for pm in result.pipeline_metrics.values():
        ...
        # Latency: accumulate _max (semantically correct rolling window)
        max_lat = pm.get("opensearch_PipelineLatency_seconds_max", -1.0)
        if max_lat >= 0:
            if result.os_pipeline_latency_max < 0:
                result.os_pipeline_latency_max = max_lat
            else:
                result.os_pipeline_latency_max = max(result.os_pipeline_latency_max, max_lat)
        # Accumulate _sum/_count for delta-based average fallback in checks
        result.os_pipeline_latency_sum += pm.get("opensearch_PipelineLatency_seconds_sum", 0.0)
        result.os_pipeline_latency_count += pm.get("opensearch_PipelineLatency_seconds_count", 0.0)

    # Compute best-effort current latency from _max when available
    if result.os_pipeline_latency_max >= 0:
        result.os_pipeline_latency = result.os_pipeline_latency_max
    # else: leave at 0.0; checks will compute from delta(sum)/delta(count)
...
async def scrape_dataprepper() -> DataPrepperScrapeResult:
    ...
    # Discover pipeline names from metric prefixes.
    # Also split DATAPREPPER_PIPELINE_NAME as comma-separated list.
    result.pipeline_names = discover_pipeline_names(result.families)
    if settings.DATAPREPPER_PIPELINE_NAME:
        for name in settings.DATAPREPPER_PIPELINE_NAME.split(","):
            name = name.strip()
            if name and name not in result.pipeline_names:
                result.pipeline_names.append(name)
...
async def _check_ingest_health(result: DataPrepperScrapeResult) -> None:
    """Probe the Data Prepper health endpoint.

    Handles:
    - JSON dict with {"status": "UP"/"OK"/"healthy"/"true"}
    - JSON string "UP" / "OK"
    - Plain text "UP" / "OK" / "healthy"
    - Empty body with HTTP 200 (treat as healthy)
    - HTTP 404: health_check_service may not expose this path → yellow, not red
    - HTTP 401/403: auth-gated health → reachable but status unknown
    """
    ca = settings.CA_CERT_PATH
    auth = _dp_ingest_auth()
    code, body, latency, error = await fetch_url(
        settings.dataprepper_health_url, ca_path=ca, auth=auth,
    )
    ...
    if code == 404:
        # /health path not exposed by health_check_service – mark as yellow in checks
        result.ingest_health_404 = True
        result.ingest_error = "health_endpoint_not_found"
        return

    if code in (401, 403):
        result.ingest_error = f"auth_failure (HTTP {code})"
        return

    if code >= 400:
        result.ingest_error = f"HTTP {code}"
        return

    # HTTP 200 (or 2xx) – parse the body
    result.ingest_healthy = _parse_health_body(body)


def _parse_health_body(body: str) -> bool:
    """Return True if the health response body indicates a healthy state."""
    if not body or not body.strip():
        # Empty 200 → treat as healthy
        return True
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            return status in ("up", "ok", "healthy", "true", "running")
        if isinstance(data, str):
            return data.lower() in ("up", "ok", "healthy", "true", "running")
        # JSON bool
    ...
    # Plain text fallback
    return stripped.lower() in ("up", "ok", "healthy", "true", "running")


def _check_dlq(result: DataPrepperScrapeResult) -> None:
    try:
...
def get_flat_metrics(result: DataPrepperScrapeResult) -> Dict[str, float]:
    return {
        ...
        # Store raw latency components for delta-based average in checks
        "os_pipeline_latency_max": result.os_pipeline_latency_max,
        "os_pipeline_latency_sum": result.os_pipeline_latency_sum,
        "os_pipeline_latency_count": result.os_pipeline_latency_count,
