...
def run_correlation_checks(
    ...
    if vector_results and dp_result and dp_result.metrics_reachable:
        dp_processed_delta = (
            dp_result.records_processed
            - pd.get("records_processed", dp_result.records_processed)
        )

        # Detect ambiguous sink resolution on any sensor
        ambiguous_sensors = [vr.sensor_ip for vr in vector_results if vr.dp_sink_ambiguous]
        configured = bool(settings.vector_dp_sink_ids)

        if ambiguous_sensors:
            n_ambiguous = len(ambiguous_sensors)
            n_total = len(vector_results)
            checks.append(CheckResult(
                id="corr.vector_dp.drop_rate",
                title="Vector→DataPrepper drop rate",
                component=C, severity="warning", status=Status.UNKNOWN,
                details=(
                    f"Vector Data Prepper sink component is unknown or ambiguous "
                    f"({n_ambiguous}/{n_total} sensors). "
                    "Set VECTOR_DATAPREPPER_SINK_COMPONENTS to the component_id of "
                    "your HTTP sink (e.g. dp_ingest)."
                ),
                remediation="Add VECTOR_DATAPREPPER_SINK_COMPONENTS=dp_ingest (or your sink name) to .env.",
            ))
        else:
            # Accumulate dp_sink_sent deltas per sensor
            total_dp_sink_delta = 0.0
            per_sensor_deltas: Dict[str, float] = {}
            all_detected: List[str] = []
            all_excluded: List[str] = []

            for vr in vector_results:
                sensor_prev = pv.get(vr.sensor_ip, {})
                prev_dp_sent = sensor_prev.get("dp_sink_sent_events", 0)
                delta = max(0.0, vr.dp_sink_sent_events - prev_dp_sent) if prev_dp_sent else 0.0
                per_sensor_deltas[vr.sensor_ip] = delta
                total_dp_sink_delta += delta
                all_detected.extend(vr.detected_dp_sink_ids)
                all_excluded.extend(vr.excluded_sink_ids)

            detected_str = ", ".join(sorted(set(all_detected))) or ("inferred" if not configured else "none")
            source_tag = "VECTOR_DATAPREPPER_SINK_COMPONENTS" if configured else "auto-inferred"
            per_sensor_str = "; ".join(
                f"{settings.sensor_display_name(ip)}: Δ{v:.0f}"
                for ip, v in per_sensor_deltas.items()
            )

            if total_dp_sink_delta > 0 and dp_processed_delta >= 0:
                drop_pct = max(0, (total_dp_sink_delta - dp_processed_delta) / total_dp_sink_delta * 100)
                ...
                checks.append(CheckResult(
                    ...
                    details=(
                        ...
                        f"(DP sink sent Δ={total_dp_sink_delta:.0f}, "
                        f"DP records_processed Δ={dp_processed_delta:.0f}). "
                        f"Sink IDs [{source_tag}]: {detected_str}. "
                        f"Per-sensor: {per_sensor_str}."
                    ),
                    remediation=(
                        "Check Data Prepper connectivity, TLS config, and buffer pressure."
                        if s != Status.GREEN else ""
                    ),
                    metadata={
                        "detected_sink_ids": sorted(set(all_detected)),
                        "excluded_ids": sorted(set(all_excluded)),
                        "dp_counter": "records_processed",
                        "sink_source": source_tag,
                    },
                ))
            elif total_dp_sink_delta == 0:
                checks.append(CheckResult(
                    id="corr.vector_dp.drop_rate",
                    title="Vector→DataPrepper drop rate",
                    component=C, severity="info", status=Status.UNKNOWN,
                    details=(
                        f"No DP-sink sent events delta yet (sink: {detected_str}). "
                        "Check will populate on next cycle."
                    ),
                ))

        # Filtering observability – always informational, never RED
        total_discarded_delta = 0.0
        total_source_delta = 0.0
        for vr in vector_results:
            sensor_prev = pv.get(vr.sensor_ip, {})
            total_discarded_delta += max(
                0.0, vr.discarded_events
                - sensor_prev.get("discarded_events", vr.discarded_events)
            )
            total_source_delta += max(
                0.0, vr.source_received_events
                - sensor_prev.get("source_received_events", vr.source_received_events)
            )
        if total_discarded_delta > 0 or total_source_delta > 0:
            filter_pct = (
                total_discarded_delta / total_source_delta * 100
                if total_source_delta > 0 else 0.0
            )
            any_intentional = any(vr.discarded_events > 0 for vr in vector_results)
            checks.append(CheckResult(
                id="corr.vector_dp.filtering",
                title="Vector intentional filtering",
                component=C, severity="info", status=Status.GREEN,
                current_value=round(filter_pct, 1),
                details=(
                    f"Intentional filter discards Δ={total_discarded_delta:.0f} "
                    f"({filter_pct:.1f}% of source received Δ={total_source_delta:.0f}). "
                    "Filtering reduction is expected and is not delivery loss."
                ),
            ))

        # Per-sensor throughput imbalance (use dp_sink_sent for a fair comparison)
        dp_sink_map: Dict[str, float] = {}
        for vr in vector_results:
            sensor_prev = pv.get(vr.sensor_ip, {})
            prev = sensor_prev.get("dp_sink_sent_events", 0)
            dp_sink_map[vr.sensor_ip] = max(0, vr.dp_sink_sent_events - prev) if prev else 0

        if len(dp_sink_map) > 1:
            values = [v for v in dp_sink_map.values() if v > 0]
            if values:
                avg = sum(values) / len(values)
                for ip, val in dp_sink_map.items():
                    if avg > 0 and val < avg * 0.2:
                        ...
                        checks.append(CheckResult(
                            ...
                            details=f"DP sink sent Δ={val:.0f} vs peer avg Δ={avg:.0f}",
